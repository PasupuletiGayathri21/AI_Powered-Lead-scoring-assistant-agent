...
def calculate_metrics(row):
    ...
    if row['demo_req']: score += 40; reasons.append("Requested Demo")
    if row['registration']: score += 10; reasons.append("Registered")
    if row['whatsapp_enquiry']: score += 15; reasons.append("WhatsApp/Call Enquiry")
    if row['pricing_check']: score += 20; reasons.append("Checked Pricing")
    

    if row['source'] == 'Referral': score += 15; reasons.append("Referral Source")
    elif row['source'] == 'Event': score += 5; reasons.append("Event Lead")
    ...
    if score >= 70: nba = "CALL IMMEDIATELY"
    elif score >= 40: nba = "Send Case Study"
    else: nba = " Add to Monthly Newsletter"
...
with st.sidebar:
    ...
    with st.form("lead_form", clear_on_submit=True):
        ...
        submit = st.button("Add to CRM")
        
        if submit and name:
            conn = sqlite3.connect('crm_data.db')
            c = conn.cursor()
            c.execute("INSERT INTO leads (name, demo_req, registration, whatsapp_enquiry, pricing_check, source, enquiry_date) VALUES (?,?,?,?,?,?,?)",
...
if not df.empty:
    ...
    def color_cells(val):
        color = 'green' if val >= 70 else 'orange' if val >= 40 else 'gray'
        return f'background-color: {color}; color: white'


    display_df = df[['name', 'Score', 'Next Action', 'Why', 'source', 'enquiry_date']]
    st.dataframe(display_df.sort_values(by='Score', ascending=False).style.applymap(color_cells, subset=['Score']), 
